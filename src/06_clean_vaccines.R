...
#Varaible that is needed and placed somwhere general
vaccinesManufacturers <- toupper(c("pfizer", "novavax","moderna","astrazeneca", "janssen", "valneva"))
vaccinesManufacturersInStudy <- toupper(c("pfizer", "novavax", "moderna", "astrazeneca", "janssen", "unk")) 
startVacinationDate <- as.Date("2020-12-27", format = "%Y-%m-%d")

###Get info form database
mydb <- dbConnect(RSQLite::SQLite(), dbConceptsFile)
Covid_vaccine <- as.data.table(dbGetQuery(mydb, "SELECT person_id, vx_record_date ,vx_admin_date ,  vx_atc, vx_type, vx_dose, vx_manufacturer FROM COV"))
setnames(Covid_vaccine, "vx_admin_date", "date")
#I assumed that date is vx_admin_date??

Covid_vaccine[, date := as.Date(date, origin = "1970-01-01")]
Covid_vaccine[, vx_record_date := as.Date(vx_record_date, origin = "1970-01-01")]
###
...
Covid_vaccine[, manufacturer_curated := toupper(vx_manufacturer)]
Covid_vaccine[!manufacturer_curated %in% vaccinesManufacturers, manufacturer_curated := "unk"]
...
# Wrong manufacturer
Covid_vaccine[removed_row == 0, manufacturer_not_in_study := fifelse(!manufacturer_curated %in% vaccinesManufacturersInStudy, 1, 0)]
...
Covid_vaccine[removed_row == 0, date_before_start_vax := fifelse(date_curated < startVacinationDate, 1, 0)]
...
Covid_vaccine <- Covid_vaccine[removed_row == 0, imputed_dose := !dose_curated %in% vx_dose]

# Dose after third
Covid_vaccine <- Covid_vaccine[removed_row == 0, dose_after_4 := fifelse(dose_curated > 4, 1, 0)]
Covid_vaccine[, removed_row := NULL]

# Clean dataset
cleanVaccines <- Covid_vaccine[, .(person_id, date, vx_record_date, vx_dose, vx_manufacturer, date_curated,
...
for (i in names(cleanVaccines)[!names(cleanVaccines) %in% c("imputed_dose", "dose_curated")]){
  cleanVaccines[is.na(get(i)), (i) := 0]
}

# Saving
saveRDS(cleanVaccines, file = paste0(d3Dir, "D3_CLEAN_VACCINES.rds"))
rm(Covid_vaccine, cleanVaccines)
